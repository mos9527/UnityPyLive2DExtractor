...
def process_namespace(
    namespace: str, typetree_defs: dict, f: TextIOWrapper, import_stmt: str = None
):
...
def __main__():
    ...
    for namespace, typetree_defs in sorted(
        ...
        if namespace:
            ...
            f = __open(f"{fname}.py")
            process_namespace(namespace, typetree_defs, f, f"from {dotss} import *")
        else:
            f = __open("__init__.py")
            process_namespace(namespace, typetree_defs, f, "")
    __open("__init__.py").write("\nTYPETREE_DEFS = " + json.dumps(namespaces, indent=4))
